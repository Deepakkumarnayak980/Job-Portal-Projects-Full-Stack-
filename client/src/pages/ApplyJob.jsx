...
const ApplyJob = () => {
  ...
  const fetchJob = async () => {
    const data = jobs.filter((jobItem) => jobItem._id.toString() === id);
    if (data.length !== 0) {
      setJobData(data[0]);
      console.log('Found job:', data[0]);
    } else {
      console.warn('No job found for the given ID:', id);
    }
  };
        ...
        <div className="bg-white text-black rounded-lg w-full">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8">
            <div className="w-full lg:w-2/3">
              ...
              <div className="rice-text" dangerouslySetInnerHTML={{ __html: JobData.description }}></div>
